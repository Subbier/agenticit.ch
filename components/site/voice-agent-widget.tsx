...
function VoiceAgentInner() {
  ...
  return (
    <div className="mx-auto mt-8 flex max-w-[460px] flex-col items-center gap-5 rounded-[22px] border border-[#E3E9F2] bg-white p-8 shadow-[0_18px_48px_rgba(11,31,58,0.12)]">
      ...
      {connected ? (
        <p className="text-[14px] font-bold text-[#0B1F3A]">
          {conversation.isSpeaking ? "Der Assistent spricht …" : "Sie sind verbunden – sprechen Sie los."}
        </p>
      ) : (
        <p className="text-[14px] font-bold text-[#0B1F3A]">Tippen Sie auf „Gespräch starten" und sprechen Sie.</p>
      )}
      ...
      {!connected ? (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-[13px] bg-gradient-to-br from-[#3BD974] to-[#22C55E] py-[15px] text-[16px] font-extrabold text-white shadow-[0_8px_20px_rgba(34,197,94,0.3)] transition hover:-translate-y-[1px] disabled:opacity-60 disabled:hover:translate-y-0"
        >
        ...
        <button
          type="button"
          onClick={stop}
          className="flex w-full items-center justify-center gap-2 rounded-[13px] bg-[#0B1F3A] py-[15px] text-[16px] font-extrabold text-white transition hover:-translate-y-[1px]"
        >
      ...
      <p className="text-[12.5px] text-[#5A6B82]">Mikrofonzugriff erforderlich · Sie hören die echte Stimme des Agenten.</p>
    </div>
  )
}
